...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Update profile row (auto-created by handle_new_user trigger) with the
  // full set of fields: account link, title, buyer_type override, email.
  await svc
    .from("profiles")
    .update({
  ...
  // Ensure a profile_accounts row exists for the new multi-account world.
  await svc
    .from("profile_accounts")
    .insert({ profile_id: created.user.id, account_id: accountId, is_default: true })
    .select()
    .maybeSingle();
