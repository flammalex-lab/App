"use client";

import { useMemo, useState } from "react";
import { useRouter } from "next/navigation";
import type {
  OrderGuideTemplate,
  OrderGuideTemplateItem,
  Product,
} from "@/lib/supabase/types";
import { Button } from "@/components/ui/Button";
import { Input } from "@/components/ui/Input";
import { useToast } from "@/components/ui/Toast";
import { CATEGORY_LABELS, BUYER_TYPE_LABELS, type BuyerType } from "@/lib/constants";

type Row = OrderGuideTemplateItem & { product: Product };

const DAYS = [
  { key: "mon", label: "Mon" },
  { key: "tue", label: "Tue" },
  { key: "wed", label: "Wed" },
  { key: "thu", label: "Thu" },
  { key: "fri", label: "Fri" },
  { key: "sat", label: "Sat" },
  { key: "sun", label: "Sun" },
];

const BUYER_TYPES: (BuyerType | "")[] = [
  "",
  "gm_restaurant",
  "gm_retail",
  "meat_buyer",
  "produce_buyer",
  "dairy_buyer",
  "cheese_buyer",
  "grocery_buyer",
];

export function TemplateEditor({
  template,
  initialItems,
  allProducts,
}: {
  template: OrderGuideTemplate;
  initialItems: Row[];
  allProducts: Product[];
}) {
  const router = useRouter();
  const toast = useToast();
  const [items, setItems] = useState<Row[]>(initialItems);
  const [meta, setMeta] = useState({
    name: template.name,
    buyer_type: template.buyer_type ?? "",
    description: template.description ?? "",
  });
  const [savingMeta, setSavingMeta] = useState(false);
  const [savingItems, setSavingItems] = useState(false);
  const [search, setSearch] = useState("");
  const [deleting, setDeleting] = useState(false);

  const existingIds = useMemo(() => new Set(items.map((i) => i.product_id)), [items]);
  const candidates = useMemo(
    () =>
      allProducts
        .filter((p) => !existingIds.has(p.id))
        .filter((p) => (search ? p.name.toLowerCase().includes(search.toLowerCase()) : true))
        .slice(0, 25),
    [allProducts, existingIds, search],
  );

  function add(product: Product) {
    setItems((xs) => [
      ...xs,
      {
        id: crypto.randomUUID(),
        template_id: template.id,
        product_id: product.id,
        suggested_qty: null,
        par_levels: null,
        sort_order: xs.length,
        product,
      },
    ]);
    setSearch("");
  }

  function remove(id: string) {
    setItems((xs) => xs.filter((x) => x.id !== id));
  }

  function setPar(id: string, day: string, val: string) {
    const qty = Number(val);
    setItems((xs) =>
      xs.map((x) => {
        if (x.id !== id) return x;
        const par = { ...(x.par_levels ?? {}) };
        if (!val || qty === 0) delete par[day];
        else par[day] = qty;
        return { ...x, par_levels: Object.keys(par).length ? par : null };
      }),
    );
  }

  async function saveMeta() {
    setSavingMeta(true);
    const res = await fetch(`/api/admin/order-guide-templates/${template.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: meta.name.trim(),
        buyer_type: meta.buyer_type || null,
        description: meta.description.trim() || null,
      }),
    });
    setSavingMeta(false);
    if (!res.ok) {
      toast.push((await res.json()).error ?? "Save failed", "error");
      return;
    }
    toast.push("Template details saved", "success");
    router.refresh();
  }

  async function saveItems() {
    setSavingItems(true);
    const res = await fetch(`/api/admin/order-guide-templates/${template.id}/items`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        items: items.map((x, idx) => ({
          product_id: x.product_id,
          suggested_qty: x.suggested_qty,
          par_levels: x.par_levels,
          sort_order: idx,
        })),
      }),
    });
    setSavingItems(false);
    if (!res.ok) {
      toast.push((await res.json()).error ?? "Save failed", "error");
      return;
    }
    toast.push(`Template saved (${items.length} items)`, "success");
    router.refresh();
  }

  async function doDelete() {
    if (!confirm("Delete this template? Buyers seeded from it keep their items; only the template is removed.")) return;
    setDeleting(true);
    const res = await fetch(`/api/admin/order-guide-templates/${template.id}`, {
      method: "DELETE",
    });
    setDeleting(false);
    if (!res.ok) {
      toast.push((await res.json()).error ?? "Delete failed", "error");
      return;
    }
    toast.push("Template deleted", "success");
    router.push("/admin/order-guides/templates");
  }

  const grouped = items.reduce<Record<string, Row[]>>((acc, r) => {
    (acc[r.product.category] ??= []).push(r);
    return acc;
  }, {});

  return (
    <div className="space-y-6">
      <section className="card p-5 space-y-3">
        <h3 className="display text-lg">Details</h3>
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="label">Name</span>
            <Input value={meta.name} onChange={(e) => setMeta({ ...meta, name: e.target.value })} />
          </label>
          <label className="block">
            <span className="label">Suggested buyer type</span>
            <select
              className="input"
              value={meta.buyer_type}
              onChange={(e) => setMeta({ ...meta, buyer_type: e.target.value })}
            >
              {BUYER_TYPES.map((t) => (
                <option key={t || "none"} value={t}>
                  {t ? BUYER_TYPE_LABELS[t as BuyerType] : "No association"}
                </option>
              ))}
            </select>
          </label>
        </div>
        <label className="block">
          <span className="label">Description</span>
          <Input
            value={meta.description}
            onChange={(e) => setMeta({ ...meta, description: e.target.value })}
          />
        </label>
        <div>
          <Button onClick={saveMeta} loading={savingMeta} size="sm">
            Save details
          </Button>
        </div>
      </section>

      <section className="card p-4">
        <h3 className="display text-lg mb-2">Add items</h3>
        <Input
          placeholder="Search catalog"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
        />
        <div className="mt-2 max-h-56 overflow-y-auto divide-y divide-black/5">
          {candidates.map((p) => (
            <button
              key={p.id}
              onClick={() => add(p)}
              className="w-full text-left p-2 hover:bg-bg-secondary flex justify-between items-center text-sm"
            >
              <span>
                {p.name}
                {p.pack_size ? (
                  <span className="text-xs text-ink-secondary"> · {p.pack_size}</span>
                ) : null}
                {!p.available_b2b ? (
                  <span className="ml-2 badge badge-gold">Not live</span>
                ) : null}
              </span>
              <span className="text-xs text-ink-secondary">{CATEGORY_LABELS[p.category]}</span>
            </button>
          ))}
          {!candidates.length ? (
            <div className="p-2 text-xs text-ink-secondary">No matches.</div>
          ) : null}
        </div>
      </section>

      {Object.entries(grouped).map(([cat, rows]) => (
        <section key={cat}>
          <h3 className="display text-lg mb-2">
            {CATEGORY_LABELS[cat as keyof typeof CATEGORY_LABELS]}
          </h3>
          <div className="card divide-y divide-black/5">
            {rows.map((r) => (
              <div key={r.id} className="p-3 space-y-2">
                <div className="flex justify-between items-baseline">
                  <div>
                    <div className="font-medium flex items-center gap-2">
                      {r.product.name}
                      {!r.product.available_b2b ? (
                        <span className="badge badge-gold">Not live</span>
                      ) : null}
                    </div>
                    <div className="text-xs text-ink-secondary">
                      {r.product.pack_size ?? ""}{" "}
                      {r.product.sku ? `· ${r.product.sku}` : ""}
                    </div>
                  </div>
                  <button
                    onClick={() => remove(r.id)}
                    className="text-xs text-feedback-error underline"
                  >
                    remove
                  </button>
                </div>
                <div className="grid grid-cols-7 gap-1 text-xs">
                  {DAYS.map((d) => (
                    <label key={d.key} className="flex flex-col items-center gap-1">
                      <span className="text-ink-secondary">{d.label}</span>
                      <Input
                        type="number"
                        min={0}
                        className="w-full text-center px-1 py-1 text-sm"
                        value={(r.par_levels as any)?.[d.key] ?? ""}
                        onChange={(e) => setPar(r.id, d.key, e.target.value)}
                      />
                    </label>
                  ))}
                </div>
              </div>
            ))}
          </div>
        </section>
      ))}

      <div className="sticky bottom-4 flex gap-2 items-center">
        <Button onClick={saveItems} loading={savingItems}>
          Save template ({items.length} items)
        </Button>
        <div className="flex-1" />
        <Button variant="danger" size="sm" loading={deleting} onClick={doDelete}>
          Delete template
        </Button>
      </div>
    </div>
  );
}
